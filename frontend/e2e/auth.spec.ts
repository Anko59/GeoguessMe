...
import {
    ...
    uniqueEmail,
    resetRateLimiter,
    waitForKeycloakVerificationLink,
} from './helpers';

async function waitForAuthCardVisuals(page: Page): Promise<void> {
    await page.locator('.auth-card').evaluate(async (element) => {
        // Let the route commit its animation before collecting it. Reading
        // getAnimations immediately after navigation can see an empty list and
        // capture the card during its first translucent frame.
...
test.describe('Local social-auth visual validation', () => {
    ...
    test('creates, names, and fully deletes a Keycloak email account', async ({ page, request }, testInfo) => {
        ...
        await page.locator('#password-confirm').fill('TestPass123!');
    ...
    test('captures Keycloak email auth screens and the optional account journey', async ({ page }, testInfo) => {
        ...
        await expect(page.getByText('Sign up with Apple', { exact: true })).toBeVisible();
        await expect(page.getByText('Sign up with GitHub', { exact: true })).toBeVisible();
    ...
    test('keeps legacy credentials hidden except for the read-only migration flow', async ({ page }, testInfo) => {
        ...
        await expect(page.getByRole('button', { name: 'Connect a Keycloak login' })).toBeVisible();
