...
export default function GroupView() {
    ...
    const loadMessages = useCallback(
        async (afterId?: string): Promise<void> => {
            if (!id) return;
            try {
                const response = await api.get<MessagesPage | Message[]>('/group/messages', {
                    params: { group_id: id, ...(afterId ? { after_id: afterId } : {}) },
                });
    ...
    useEffect(() => {
        ...
        void api
            ...
            .catch((requestError: unknown) => setError(getAPIErrorMessage(requestError, 'Unable to load group')));
    }, [id]);
        ...
        const connect = async (): Promise<void> => {
            ...
            try {
                ...
                const socket = new WebSocket(
                    `${protocol}//${window.location.host}/api/v1/ws?group_id=${encodeURIComponent(id)}&ticket=${encodeURIComponent(ticketResponse.data.ticket)}`,
                );
                ws.current = socket;
                socket.onopen = () => {
