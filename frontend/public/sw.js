...
// Treat payloads as untrusted even though normal messages are encrypted: a
// compromised subscription record must never turn a notification click into
// an external-navigation primitive.
function safeTargetUrl(value) {
    try {
        const parsed = new URL(typeof value === 'string' ? value : '/groups', self.location.origin);
        if (parsed.origin !== self.location.origin) return '/groups';
...
function openTargetUrl(targetUrl) {
    if (targetUrl) {
        ...
        const absoluteUrl = new URL(safeUrl, self.location.origin).href;
