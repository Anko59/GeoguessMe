...
func handlerConfig() *config.Config {
	return &config.Config{
		...
		ViewWindow: time.Minute, GuessWindow: 2 * time.Minute, LocationHide: 48 * time.Hour, PhotoRetention: 24 * time.Hour, AllowedOrigins: []string{"http://localhost:8080"},
	}
}
...
func TestGroupAndReadHandlers(t *testing.T) {
	...
	mock.ExpectQuery(`(?s)SELECT p\.id, p\.created_at, g\.user_id, g\.score.*WHERE TRUE AND NOT g\.timed_out AND g\.group_id = \$1`).WithArgs(group.ID).
