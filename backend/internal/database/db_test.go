...
func TestMigrationDiscoveryAndDisconnectedDatabase(t *testing.T) {
	all, err := migrations()
	if err != nil || len(all) != 4 || all[0].Version != 1 || all[1].Version != 2 || all[2].Version != 3 || all[3].Version != 4 {
...
func TestMigrationStatusUsesPool(t *testing.T) {
	...
	if err != nil || len(records) != 4 || !records[0].Applied || records[1].Applied || records[2].Applied || records[3].Applied {
...
func TestMigrateUpSkipsAppliedMigrations(t *testing.T) {
	...
	mock.ExpectQuery("SELECT version FROM schema_migrations").WillReturnRows(
		pgxmock.NewRows([]string{"version"}).AddRow(1).AddRow(2).AddRow(3).AddRow(4),
	)
