...
func TestCompleteChallengeProcessing(t *testing.T) {
	...
	mock.ExpectExec("UPDATE media_processing_jobs.*status = 'ready'").WithArgs("job-1", "photos/canonical-1", "photo", "photo-1", "video/mp4", int64(12)).WillReturnResult(pgxmock.NewResult("UPDATE", 1))
	mock.ExpectCommit()

	if err := repo.CompleteChallengeProcessing(ctx, "job-1", photos, "video/mp4", 12); err != nil {
...
func TestCompleteChallengeProcessingRollsBackOnPhotoInsertFailure(t *testing.T) {
	...
	if err := repo.CompleteChallengeProcessing(ctx, "job-1", []*models.Photo{photo}, "video/mp4", 12); !errors.Is(err, photoErr) {
...
func TestCompleteChallengeProcessingRejectsEmptyPhotos(t *testing.T) {
	mock := newMockPool(t)
	repo := NewRepository(mock)
	if err := repo.CompleteChallengeProcessing(context.Background(), "job-1", nil, "video/mp4", 0); err == nil {
...
func TestCompleteChatProcessing(t *testing.T) {
	...
	mock.ExpectExec("UPDATE media_processing_jobs.*status = 'ready'").WithArgs("job-2", "chat-media/canonical-1", "chat", "msg-1", "video/mp4", int64(12)).WillReturnResult(pgxmock.NewResult("UPDATE", 1))
	mock.ExpectCommit()

	if err := repo.CompleteChatProcessing(ctx, "job-2", msg, asset); err != nil {
...
func TestCompleteChatProcessingRollsBackOnJobUpdateFailure(t *testing.T) {
	...
	mock.ExpectExec("INSERT INTO chat_media").WithArgs(asset.ID, asset.GroupID, asset.UserID, asset.StorageKey, asset.MIMEType, asset.ByteSize, asset.CreatedAt).WillReturnResult(pgxmock.NewResult("INSERT", 1))
	mock.ExpectExec("INSERT INTO messages").WithArgs(msg.ID, msg.GroupID, msg.UserID, asset.ID, msg.ReplyToID, msg.Content, msg.CreatedAt).WillReturnResult(pgxmock.NewResult("INSERT", 1))
	mock.ExpectExec("UPDATE media_processing_jobs.*status = 'ready'").WithArgs("job-2", "chat-media/canonical-1", "chat", "msg-1", "video/mp4", int64(12)).WillReturnError(updateErr)
	mock.ExpectRollback()

	if err := repo.CompleteChatProcessing(ctx, "job-2", msg, asset); !errors.Is(err, updateErr) {
...
func TestCompleteProcessingJobTx(t *testing.T) {
	...
	if err := repo.CompleteProcessingJobTx(ctx, tx, "job-1", "photos/canonical-1", "photo", "photo-1", "video/mp4", 12); err != nil {
