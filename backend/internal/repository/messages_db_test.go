...
func TestChatMediaMessagePersistenceAndLookup(t *testing.T) {
	...
	mock.ExpectCommit()
	if err := CreateChatMediaMessage(context.Background(), message, asset); err != nil {
	...
	mock.ExpectCommit()
	if err := CreateChatMediaMessage(context.Background(), replyMessage, replyAsset); err != nil {
	...
	loaded, err := GetChatMedia(context.Background(), asset.ID)
	...
	missing, err := GetChatMedia(context.Background(), "unattached")
	...
	for _, candidate := range invalid {
		if err := CreateChatMediaMessage(context.Background(), candidate, asset); err == nil {
			t.Fatalf("invalid message %#v was accepted", candidate)
		}
	}
	if err := CreateChatMediaMessage(context.Background(), message, nil); err == nil {
