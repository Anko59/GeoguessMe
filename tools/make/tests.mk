...
qa-agent: ## Run the local LLM-driven source-blind QA agent against deployed dev.
	$(MAKE) bootstrap-e2e
	@QA_BASE_URL="$(QA_BASE_URL)" QA_BUILD_SHA="$(QA_BUILD_SHA)" QA_BUDGET="$(QA_BUDGET)" QA_RUNTIME="$(QA_RUNTIME)" QA_REPORT_DIR="$(abspath $(QA_REPORT_DIR))" QA_MAILBOX_PROVIDER="$(QA_MAILBOX_PROVIDER)" QA_MAILBOX_API_URL="$(QA_MAILBOX_API_URL)" QA_MAILBOX_ADDRESS="$(QA_MAILBOX_ADDRESS)" QA_ACCOUNT_PASSWORD="$(QA_ACCOUNT_PASSWORD)" QA_ACCOUNT_OWNER_USERNAME="$(QA_ACCOUNT_OWNER_USERNAME)" QA_ACCOUNT_MEMBER_USERNAME="$(QA_ACCOUNT_MEMBER_USERNAME)" QA_ACCOUNT_OUTSIDER_USERNAME="$(QA_ACCOUNT_OUTSIDER_USERNAME)" \
...
qa-browser-mcp: ## Run the Dockerized provider-neutral browser MCP server.
	...
	@$(COMPOSE_TOOLS) run -T --rm --no-deps --user "$(shell id -u):$(shell id -g)" \
		...
		-e QA_MAILBOX_PROVIDER -e QA_MAILBOX_API_URL -e QA_MAILBOX_ADDRESS -e QA_FAKE_LATITUDE -e QA_FAKE_LONGITUDE -e QA_FAKE_LOCATION_ACCURACY \
