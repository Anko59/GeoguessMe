...
# The eight harness self-tests are path-triggered in preflight via the
...
for dep in test-makefile-fragments-regression test-structure-regression test-debt-markers-regression test-docs-agent-config test-ci-classifier test-e2e-regression test-dev-workflow-regression test-load-harness-regression; do
